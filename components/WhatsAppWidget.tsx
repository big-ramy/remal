...
const WhatsAppWidget = () => {
    ...
    const handleOpenChat = () => {
        ...
        setMessages([
            { id: 1, sender: 'system', text: textData.welcome1[language] },
            { id: 2, sender: 'system', text: textData.welcome2[language] }
        ]);
        ...
        setTimeout(() => {
            setMessages(prev => [
                ...prev,
                { id: 3, sender: 'bot', text: textData.welcome3[language] }
            ]);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            const botResponseText = await askGemini(userInput);
    ...
    const textData = {
        bubble1: { ar: "مرحباً، معك موظف خدمة العملاء", en: "Hello, customer service here" },
        bubble2: { ar: "سعيدين جداً بزيارتك، كيف نقدر نخدمك اليوم؟", en: "We're so happy you visited, how can we help you today?" },
        chatHeader: { ar: "الرمال الذهبية", en: "Golden Sands" },
        chatSubheader: { ar: "خدمة العملاء", en: "Customer Service" },
        inputPlaceholder: { ar: "اكتب رسالتك هنا...", en: "Type your message here..." },
        welcome1: { ar: "مرحباً، معك موظف خدمة العملاء", en: "Hello, customer service here" },
        welcome2: { ar: "سعيدين جداً بزيارتك، كيف نقدر نخدمك اليوم؟", en: "We're so happy you visited, how can we help you today?" },
        welcome3: { ar: "حياك الله يا غالي، شركة الرمال الذهبية تقدم لك أفضل حلول التنقل العمودية الحديثة بجودة وكفاءة وأمان.", en: "Welcome! Golden Sands offers you the best modern vertical transportation solutions with quality, efficiency, and safety." },
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setIsOpen(false)}>
                    <div onClick={e => e.stopPropagation()} className="w-full max-w-sm h-[70vh] flex flex-col bg-gray-100 dark:bg-secondary-light rounded-lg shadow-2xl m-4">
                        ...
                        <form onSubmit={handleSendMessage} className="p-4 bg-gray-100 dark:bg-secondary-light border-t dark:border-gray-600 flex items-center gap-2">
                            ...
                            />
                            <button type="submit" className="bg-green-500 text-white p-3 rounded-full hover:bg-green-600 transition-colors">
                                <SendIcon />
                            </button>
                        </form>
                    </div>
